...
if __name__ == "__main__":
  model = Transformer(65, 256, 4, 128, 16, 32)
  X_train, Y_train, X_test, Y_test = make_dataset()
  lr = 0.003
  for i in range(10):
    optim = Adam(get_parameters(model), lr=lr)
    train(model, X_train, Y_train, optim, 50, BS=128, allow_jit=True)
    # acc, Y_test_preds = evaluate(model, X_test, Y_test, num_classes=65, return_predict=True)
    lr /= 1.2
